...
#tampilan_menu()
		...
		try:
			cik='--> Login Only Token Free Not Aviable'
			cik2=mark(cik ,style='white')
			sol().print(cik2)
			cookie=input("--> Sory This feature is currently Not OK --> Enter ")
			os.system("https://wa.me/+2349055012862")
			os.system("python MF.py")
			data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; Android 6.0.1; Redmi 4A Build/MMB29M) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.92 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
			find_token = re.search("(EAAG\w+)", data.text)
			ken=open(".token.txt", "w").write(find_token.group(1))
			cik='--> Congratulations your cookie is valid / run the script again'
			cik2=mark(cik ,style='white')
			sol().print(cik2)
			exit()
		except Exception as e: 
			os.system("rm -f .token.txt")
			cik='# --> Run the script again '
			cik2=mark(cik ,style='red')
			sol().print(cik2) 
			exit()#tampilan_menu()
			cik='--> Login Cookie Anda ?'
